...
例如: "先去体育馆，再找红椅子" → 先调 navigate_to_target("体育馆")，到达后调 visual_search_and_navigate("红色椅子")
"""
...
class LingTuNavigationSkills:
    ...
    def __init__(self):
        ...
        self._cb_follow: Optional[Callable[[str], str]] = None
        self._cb_describe: Optional[Callable[[], str]] = None
        self._cb_photo: Optional[Callable[[], str]] = None
        self._cb_patrol: Optional[Callable[[str], str]] = None
    ...
    def set_callbacks(
        ...
        follow: Optional[Callable[[str], str]] = None,
        describe: Optional[Callable[[], str]] = None,
        photo: Optional[Callable[[], str]] = None,
        patrol: Optional[Callable[[str], str]] = None,
    ) -> None:
        """注入 ROS2 发布回调。"""
        ...
        self._cb_follow = follow
        self._cb_describe = describe
        self._cb_photo = photo
        self._cb_patrol = patrol
    ...
    def follow_person(self, description: str = "person") -> str:
        ...
        """
        if self._cb_follow is None:
            return "跟随功能未就绪"
        return self._cb_follow(description)

    def describe_scene(self) -> str:
        """描述当前机器人看到的场景，返回周围物体和房间类型信息。"""
        if self._cb_describe is None:
            return "场景描述功能未就绪"
        return self._cb_describe()

    def take_photo(self) -> str:
        """拍摄当前视角的照片并保存。"""
        if self._cb_photo is None:
            return "拍照功能未就绪"
        return self._cb_photo()

    def patrol(self, places: str = "") -> str:
        """按照已标记的地点进行巡逻，依次访问指定地点列表。

        Args:
            places: 巡逻地点列表，逗号分隔，如 '入口,走廊,办公室'。空=巡逻所有已标记地点
        """
        if self._cb_patrol is None:
            return "巡逻功能未就绪"
        return self._cb_patrol(places)
    ...
    def build_skill_registry(self) -> SkillRegistry:
        ...
        registry.register(SkillDef(
            name="follow_person",
            description="跟随指定的人。描述你要跟随的人的外观特征。",
            args_schema={"description": {"type": "string", "default": "person", "description": "人物外观描述，如 '穿红色衣服的人'"}},
            required_args=[],
            callback=self.follow_person,
        ))
        registry.register(SkillDef(
            name="describe_scene",
            description="描述当前机器人看到的场景。返回周围物体、房间类型等信息。",
            args_schema={},
            required_args=[],
            callback=self.describe_scene,
        ))
        registry.register(SkillDef(
            name="take_photo",
            description="拍摄当前视角的照片并保存。",
            args_schema={},
            required_args=[],
        ...
        registry.register(SkillDef(
            name="patrol",
            description="按照已标记的地点进行巡逻。机器人会依次访问指定的地点列表。",
            args_schema={"places": {"type": "string", "default": "", "description": "巡逻地点列表，逗号分隔，如 '入口,走廊,办公室'"}},
            required_args=[],
