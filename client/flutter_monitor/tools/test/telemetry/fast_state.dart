...
void main(List<String> args) async {
  ...
  int count = 0;
  bool hasJoints = false, hasIMU = false, hasPose = false, hasRPY = false;
  ...
  try {
    await for (final s in c.streamFastState(FastStateRequest()..desiredHz = 10.0)
      ...
      if (s.jointAngles.isNotEmpty) hasJoints = true;
      ...
      if (count == 1) {
        ...
        print('  Vel:  ${s.velocity.linear.x.toStringAsFixed(3)} m/s');
        print('  Joints: ${s.jointAngles.length} DOF');
  ...
  print('\n$count frames (${(count / dur).toStringAsFixed(1)} Hz)');
  print('Pose: ${hasPose ? "YES" : "NO"}  RPY: ${hasRPY ? "YES" : "NO"}  Joints: ${hasJoints ? "YES" : "NO"}  IMU: ${hasIMU ? "YES" : "NO"}');
  await ch.shutdown();
  exit(0);
}
