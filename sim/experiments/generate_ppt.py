# -*- coding: utf-8 -*-
"""
HSG-Nav project presentation PPT generator.
Generates a complete project defense/showcase PPT including:
  - Cover slide
  - Research background and motivation
  - System architecture
  - Core innovations (6 innovation points)
...
# ── Color theme ──
BG_DARK    = RGBColor(0x1A, 0x1A, 0x2E)   # dark blue-black background
BG_CARD    = RGBColor(0x16, 0x21, 0x3E)   # card deep blue
ACCENT     = RGBColor(0x00, 0xD2, 0xFF)   # tech blue
ACCENT2    = RGBColor(0x0F, 0xCB, 0x8A)   # green accent
ACCENT3    = RGBColor(0xFF, 0x6B, 0x6B)   # red accent
...
def make_background_slide(prs):
    """Slide 2: Research background and motivation"""
...
def make_innovation1_slide(prs):
    """Slide 4: Innovation 1 -- Online hierarchical scene graph"""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_bg(slide, BG_DARK)
...
def make_innovation2_slide(prs):
    """Slide 5: Innovation 2 -- Fast-Slow dual-path reasoning"""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_bg(slide, BG_DARK)
...
def make_innovation3_slide(prs):
    """Slide 6: Innovation 3 -- Belief-Aware reasoning (BA-HSG)"""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_bg(slide, BG_DARK)
...
def make_innovation4_slide(prs):
    """Slide 7: Innovation 4 -- Topological semantic exploration (TSG)"""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_bg(slide, BG_DARK)
...
def make_comparison_slide(prs):
    """Slide 9: Comparison with existing work"""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_bg(slide, BG_DARK)
