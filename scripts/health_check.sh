...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
if [ "$NODE_COUNT" -gt 0 ]; then
    TOPICS=("/nav/odometry" "/nav/terrain_map" "/nav/path" "/cmd_vel" "/nav/dog_odometry" "/nav/adapter_status")
...
if command -v ros2 &> /dev/null && [ "$NODE_COUNT" -gt 0 ]; then
    ...
    ACTIVE_NODES=$(timeout 3 ros2 node list 2>/dev/null)
    NODE_FAIL=0
    ...
    # dog 模式检查
    if echo "$ACTIVE_NODES" | grep -q "/han_dog_bridge"; then
        check_pass "/han_dog_bridge (dog mode)"
    elif echo "$ACTIVE_NODES" | grep -q "/robot_driver"; then
        check_pass "/robot_driver (generic mode)"
    else
        check_warn "无驱动节点运行"
    fi
    FAIL_COUNT=$((FAIL_COUNT + NODE_FAIL))
else
    check_info "跳过节点存活检查（系统未运行）"
fi
