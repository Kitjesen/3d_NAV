#!/usr/bin/env python3
"""
HSG-Nav real-robot evaluation framework.

Runs an instruction set automatically, records navigation results, and
computes SR/SPL metrics.
...
Dependencies:
  ...
  - JSON instruction set file (instruction_set.json)

Output:
  results/<timestamp>_<level>_<ablation>/
    ├── raw_logs/       # detailed log per instruction
...
class TrialResult:
    ...
    def compute_spl(self):
        """Compute SPL = SR * (shortest / max(actual, shortest))."""
        if not self.success:
            self.spl = 0.0
            return
...
def compute_shortest_path(start: Dict, target: Dict) -> float:
    """Straight-line distance between two points (lower bound on path length)."""
    dx = target["x"] - start["x"]
    dy = target["y"] - start["y"]
...
class EvalRunner:
    """Evaluation runner (works with ROS2 or offline mock)."""
    ...
    def get_instructions(self, level: str) -> List[Dict]:
        """Return the instruction list for the given level."""
        if level == "all":
            all_instr = []
    ...
    def run_single_trial(
        ...
        """
        Run one trial for a single instruction.

        On a real robot this method:
          1. Moves the robot to start_position
          2. Publishes the instruction to /instruction
          3. Monitors /status until completion or timeout
          4. Records final position, path length, etc.

        Currently an offline mock framework; replace ROS2 calls for real-robot runs.
        """
        result = TrialResult(
        ...
        if ros_node is not None:
            ...
            # Offline mock: mark as not executed
    ...
    def _run_with_ros(self, result: TrialResult, instr: Dict, node) -> TrialResult:
        """Execute real-robot navigation via ROS2 (requires running system)."""
        try:
            import rclpy
    ...
    def run_level(
        ...
        """Run all instructions for a level, repeating each trial times."""
    ...
    def summarize(results: List[TrialResult], level: str) -> LevelSummary:
        """Aggregate a list of trial results."""
