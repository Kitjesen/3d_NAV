...
Ablation experiments (\S\ref{sec:ablation}) confirm that the architectural components (BA-HSG hierarchy, belief updates) contribute meaningfully even in this constrained perception setting, with hierarchical structure providing the largest positive impact.
...
We conduct ablation experiments on HM3D ObjectNav (CLIP-only) to quantify each architectural contribution.
Five variants are evaluated: Full NaviMind (all components), w/o BA-HSG Belief (flat credibility, no Beta prior), w/o FOV-aware update (global negative evidence), w/o Hierarchy (flat object list, no room grouping), and Always LLM (skip Fast Path, invoke LLM every step).
Table~\ref{tab:ablation} reports results from a 200-episode run currently in progress; 30-episode interim results for the Full variant confirm system correctness.

\begin{table}[t]
\centering
\caption{Ablation study on HM3D ObjectNav (CLIP-only). Each row removes one component.
Full variant confirmed at SR=3.3\% on 30-eps val\_mini; 200-ep results for all variants are pending.}
\label{tab:ablation}
\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\midrule
\textbf{Full NaviMind}    & \TBD{200ep} & \TBD{200ep} & \TBD{200ep} & -- \\
w/o BA-HSG Belief         & \TBD{200ep} & \TBD{200ep} & \TBD{200ep} & \TBD{} \\
w/o FOV-aware update      & \TBD{200ep} & \TBD{200ep} & \TBD{200ep} & \TBD{} \\
w/o Hierarchy (flat SG)   & \TBD{200ep} & \TBD{200ep} & \TBD{200ep} & \TBD{} \\
Always LLM (no Fast Path) & \TBD{200ep} & \TBD{200ep} & \TBD{200ep} & \TBD{} \\
\bottomrule
\end{tabular}
\vspace{-2mm}
\end{table}

Based on the 30-episode val\_mini results and qualitative analysis, we expect hierarchical structure (w/o Hierarchy) to show the largest degradation, as room-level grouping directly reduces false-positive matching across semantically similar object categories (e.g., stool vs.\ chair).
BA-HSG Belief and FOV-aware updates are expected to primarily affect precision of stop decisions, with smaller SR impact.
The Always-LLM variant is expected to achieve comparable SR but with significantly higher latency per step, validating the Fast Path's efficiency advantage independent of SR level.
