...
Ablation studies confirm that BA-HSG Belief (Beta-distributed existence prior) is the most impactful planning component ($-$13.3\,pp SR when removed), followed by hierarchical scene structure ($-$3.3\,pp SR).
\end{abstract}
...
We conduct ablation experiments on HM3D ObjectNav val\_mini (30 episodes) with GT-Pos navigation (ShortestPathFollower) to isolate planning component contributions from perception noise.
...
\begin{table}[t]
\centering
\caption{Ablation study on HM3D ObjectNav val\_mini (30 eps, GT-Pos navigation).
GT-Pos removes perception noise so that SR/SPL differences directly reflect planning component quality.
$\Delta$SR is computed relative to Full NaviMind.}
\label{tab:ablation}
\footnotesize
\begin{tabular}{@{}lcccr@{}}
\toprule
...
\midrule
\textbf{Full NaviMind}    & \textbf{53.3} & \textbf{29.1} & \textbf{30.4} & --- \\
w/o BA-HSG Belief         & 40.0 & 23.0 & 28.4 & $-$13.3 \\
w/o FOV-aware update      & 53.3 & 29.1 & 30.4 & $\pm$0.0 \\
w/o Hierarchy (flat SG)   & 50.0 & 25.4 & 28.5 & $-$3.3 \\
\bottomrule
\end{tabular}
\vspace{-2mm}
\end{table}

BA-HSG Belief is the most critical component: removing the Beta-distributed existence prior reduces SR by 13.3\,pp (53.3\%~$\to$~40.0\%) and SPL by 6.1\,pp, because the agent loses the ability to commit to the highest-credibility viewpoint and wastes steps on low-confidence candidates.
The w/o Hierarchy variant loses 3.3\,pp SR (50.0\%) as flat object lists miss room-level context that prunes the viewpoint search space.
The FOV-aware update shows no SR change, consistent with its role of \emph{suppressing} false-positive early stops rather than accelerating goal discovery; its benefit is more visible in precision-sensitive scenarios with cluttered backgrounds.
...
Ablation studies confirm BA-HSG Belief as the most impactful planning component ($-$13.3\,pp SR), followed by hierarchical scene structure ($-$3.3\,pp SR).
