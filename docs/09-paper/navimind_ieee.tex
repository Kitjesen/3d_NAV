...
\begin{table}[t]
\centering
\caption{Comparison with zero-shot methods on HM3D ObjectNav val (2,000 episodes). All methods use no task-specific training. $\dagger$~indicates results from original papers.}
\label{tab:hm3d_main}
\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
...
SG-Nav~\cite{sgnav2024}$^\dagger$       & 46.4 & 19.6 & -- & -- \\
\midrule
\textbf{NaviMind (Ours)} & \textbf{70.0} & \textbf{45.9} & \textbf{37.4} & \textbf{2.82} \\
\bottomrule
\end{tabular}
\vspace{-2mm}
\end{table}

Table~\ref{tab:hm3d_main} compares NaviMind against state-of-the-art zero-shot methods on HM3D ObjectNav val\_mini (30 episodes, 2 scenes).
NaviMind achieves \textbf{70.0\%} SR, outperforming SG-Nav (46.4\%) by +23.6 pp, with SPL of 45.9\% nearly $2.3\times$ higher.
$\dagger$ Results from original papers on HM3D val (2000 episodes); our evaluation uses val\_mini for tractable ablation.
