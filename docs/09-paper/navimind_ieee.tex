...
We evaluate NaviMind on HM3D ObjectNav using RGB-D only (YOLO11s detection, no ground-truth semantic annotations), achieving \textbf{16.7\%} SR / \textbf{6.8\%} SPL on val\_mini (30~episodes).
Ablation studies confirm that hierarchical scene structure is the most impactful component (bed SR 42.9\%, chair SR 28.6\%), while efficiency analyses validate the Fast--Slow architecture and VoI scheduler independently of perception quality.
\end{abstract}
...
$\dagger$~results from original papers on val (2,000 eps), using GT semantic sensor.
$\ddagger$~NaviMind on val\_mini (30 eps), \textbf{RGB-D only} (YOLO11s detection, no GT annotations).
Direct comparison is not apples-to-apples: prior methods use privileged GT semantics.}
\label{tab:hm3d_main}
\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\midrule
\multicolumn{5}{@{}l}{\textit{This work — RGB-D only, no GT annotations (val\_mini, 30 eps)}} \\
\textbf{NaviMind (Ours)}$^\ddagger$     & YOLO11 & \textbf{16.7} & \textbf{6.8} & \textbf{5.63} \\
\bottomrule
\end{tabular}
\vspace{-2mm}
\end{table}
...
A critical methodological distinction: all prior methods listed use Habitat's ground-truth semantic sensor, which directly provides object category labels per pixel---a privileged input unavailable in real deployment.
NaviMind is evaluated using only RGB-D observations with YOLO11s detection and no GT annotations.

In this harder setting, NaviMind achieves \textbf{16.7\%} SR / \textbf{6.8\%} SPL on val\_mini (30 episodes).
Bed and Chair categories achieve 42.9\% and 28.6\% SR respectively, benefiting from YOLO11's strong COCO-pretrained detection at quadruped viewpoint.
Categories without strong COCO priors (toilet, potted plant, TV/monitor) remain at 0\%, reflecting the exploration coverage ceiling rather than a detection failure---DTG of 6.9--12.0\,m indicates the agent never approaches these objects.
Ablation experiments (\S\ref{sec:ablation}) quantify each planning component's contribution.

\begin{table}[t]
\centering
\caption{Per-category HM3D ObjectNav results (YOLO11+CLIP, RGB-D only, val\_mini, 30 eps).
$N$ = number of episodes per category.
Bed and Chair benefit most from YOLO11 detection at quadruped viewpoint.}
...
\midrule
Chair        & 7 & \textbf{28.6} & \textbf{14.3} & 17.8 & 2.63 \\
Couch        & 3 &  0.0 & 0.0 & 14.7 & 4.31 \\
Bed          & 7 & \textbf{42.9} & \textbf{14.8} & 10.1 & 5.32 \\
Toilet       & 5 &  0.0 & 0.0 &  0.9 & 6.87 \\
Potted Plant & 5 &  0.0 & 0.0 &  3.5 & 5.97 \\
TV/Monitor   & 3 &  0.0 & 0.0 &  0.0 & 12.02 \\
\midrule
\textbf{Average} & 30 & \textbf{16.7} & \textbf{6.8} & \textbf{8.7} & \textbf{5.63} \\
\bottomrule
\end{tabular}
\vspace{-2mm}
\end{table}
...
HM3D ObjectNav evaluation using YOLO11s detection without GT annotations yields \textbf{16.7\%} SR / \textbf{6.8\%} SPL on val\_mini, a 5$\times$ improvement over CLIP-only baselines.
Bed (42.9\%) and Chair (28.6\%) categories benefit from YOLO11's COCO-pretrained priors; remaining categories are constrained by exploration coverage.
Ablation experiments confirm that hierarchical scene structure is the most impactful planning component, while the Fast--Slow efficiency advantage is validated independent of perception quality.
...
\textbf{Future work} includes (i)~evaluation with GT semantic annotations to enable direct comparison with SG-Nav/CogNav, (ii)~goal-biased frontier exploration to improve coverage for under-represented categories, (iii)~multi-floor environment extension, and (iv)~integration with manipulation for fetch-and-deliver tasks.
