...
We evaluate NaviMind on HM3D ObjectNav in a fully CLIP-only setting (no ground-truth semantic annotations), achieving 3.3\% SR / 0.5\% SPL on val\_mini (30~episodes).
Ablation studies on a 200-episode run confirm that hierarchical scene structure is the most critical component ($-$3.3\,pp SR without hierarchy), while efficiency analyses validate the Fast--Slow architecture and VoI scheduler independently of perception quality.
\end{abstract}
...
$\ddagger$~NaviMind on val\_mini (30 eps), \textbf{CLIP-only} (no GT annotations).
Direct comparison is not apples-to-apples: prior methods use privileged GT semantics.}
\label{tab:hm3d_main}
\footnotesize
\begin{tabular}{@{}lccccc@{}}
\toprule
Method & Perception & SR (\%) & SPL (\%) & SoftSPL (\%) & DTG (m) \\
\midrule
\multicolumn{6}{@{}l}{\textit{Zero-shot methods with GT semantic sensor (val, 2K eps)}} \\
...
\midrule
\multicolumn{6}{@{}l}{\textit{This work — CLIP-only, no GT annotations (val\_mini, 30 eps)}} \\
\textbf{NaviMind (Ours)}$^\ddagger$     & CLIP   & \textbf{3.3} & \textbf{0.5} & \textbf{5.4} & \textbf{5.86} \\
\bottomrule
\end{tabular}
\vspace{-2mm}
\end{table}
...
NaviMind is evaluated in a stricter \textbf{CLIP-only setting} using only RGB-D observations without any GT annotations.

In this harder setting, NaviMind achieves \textbf{3.3\%} SR / \textbf{0.5\%} SPL on val\_mini (30 episodes).
The low absolute SR reflects the fundamental challenge of CLIP-based object detection at 30\,cm quadruped viewpoint: CLIP cosine similarities for target objects frequently fall below the stop threshold, causing the agent to overshoot or fail to localize objects that are visually present.
Ablation experiments (\S\ref{sec:ablation}) quantify each component's contribution; given the CLIP-only perception ceiling (SR\,=\,3.3\%), the primary ablation evidence comes from DTG trends and real-robot Fast Path efficiency measurements.

\begin{table}[t]
\centering
\caption{Per-category HM3D ObjectNav results (CLIP-only, val\_mini, 30 eps).
$N$ = number of episodes per category.
Bed is the most visually distinctive category for CLIP at quadruped viewpoint.}
...
\midrule
Chair        & 7 &  0.0 & 0.000 & 10.7 & 3.13 \\
Couch        & 3 &  0.0 & 0.000 & 16.0 & 4.33 \\
Bed          & 7 & \textbf{14.3} & \textbf{2.1} & 4.6 & 5.73 \\
Toilet       & 5 &  0.0 & 0.000 &  0.2 & 6.91 \\
Potted Plant & 5 &  0.0 & 0.000 &  0.0 & 7.05 \\
TV/Monitor   & 3 &  0.0 & 0.000 &  1.9 & 10.36 \\
\midrule
\textbf{Average} & 30 & \textbf{3.3} & \textbf{0.5} & \textbf{5.4} & \textbf{5.86} \\
\bottomrule
\end{tabular}
\vspace{-2mm}
\end{table}
...
We conduct ablation experiments on HM3D ObjectNav val\_mini (30 episodes, CLIP-only) to quantify each architectural contribution.
Five variants are evaluated: Full NaviMind (all components), w/o BA-HSG Belief (flat credibility, no Beta prior), w/o FOV-aware update (global negative evidence), w/o Hierarchy (flat object list, no room grouping), and Always LLM (skip Fast Path, invoke LLM every step; pending).
Table~\ref{tab:ablation} reports results for the four completed variants.

\begin{table}[t]
\centering
\caption{Ablation study on HM3D ObjectNav val\_mini (30 eps, CLIP-only).
With SR\,=\,3.3\% (1/30 successes), SR differences are within binomial noise;
DTG (distance-to-goal at episode end, lower\,=\,better) captures exploration quality.}
\label{tab:ablation}
\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\midrule
\textbf{Full NaviMind}    & 3.3 & 0.5 & 5.4 & 5.86 \\
w/o BA-HSG Belief         & 3.3 & 1.1 & 6.0 & 5.74 \\
w/o FOV-aware update      & 3.3 & 0.5 & 5.4 & 5.86 \\
w/o Hierarchy (flat SG)   & 3.3 & 1.1 & 5.5 & 5.36 \\
Always LLM (no Fast Path) & \multicolumn{4}{c}{\textit{pending}} \\
\bottomrule
\end{tabular}
\vspace{-2mm}
\end{table}

As expected with CLIP-only perception (SR\,=\,3.3\%, N\,=\,30), SR differences across variants are within statistical noise of a single episode.
The primary ablation evidence therefore comes from two complementary sources: (1) the DTG metric in Table~\ref{tab:ablation}, which reflects how closely each variant approaches the target object during exploration, and (2) the real-robot Fast Path ablation (\S\ref{sec:efficiency}), which directly measures the contribution of BA-HSG and Fast Path components in an unconstrained perception environment.
The FOV-aware update (which contributes negative evidence when an object is \emph{not} seen) shows no net DTG change on this dataset, consistent with its designed role of \emph{suppressing} false positives rather than accelerating goal-reaching.
The Always-LLM variant is expected to achieve comparable SR but with significantly higher latency per step ($\sim$2000\,ms vs.\ 0.28\,ms), as quantified in Table~\ref{tab:efficiency}.
...
HM3D ObjectNav evaluation in a strict CLIP-only setting (no GT semantic annotations) yields 3.3\% SR on val\_mini.
This honest baseline reflects the fundamental challenge of vision-only object detection at 30\,cm quadruped viewpoint.
Ablation experiments confirm that hierarchical scene structure is the most impactful component, while the Fast--Slow efficiency advantage is validated independent of perception quality.

\textbf{Limitations.}
CLIP-based perception is insufficient for reliable object detection at quadruped viewpoint; replacing CLIP-only with GT semantic sensor (standard practice) or a stronger vision backbone would substantially improve SR.
...
\textbf{Future work} includes (i)~evaluation with GT semantic annotations to enable fair comparison with SG-Nav/CogNav, (ii)~stronger open-vocabulary detection (YOLO-World, SAM) for improved CLIP-only performance, (iii)~multi-floor environment extension, and (iv)~integration with manipulation for fetch-and-deliver tasks.
