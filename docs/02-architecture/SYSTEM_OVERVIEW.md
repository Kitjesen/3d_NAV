...
L1  Hardware      ThunderDriver (gRPC → brainstem) / sim drivers
...
| `exploration(backend)` | `tare` (CMU TARE planner via NativeModule) or `none`. Wavefront frontier was removed from this stack on 2026-04-25 — it still exists as `WavefrontFrontierExplorer` invoked from `navigation()` when `enable_frontier=True`. |
...
- `SafetyRingModule.stop_cmd → driver.stop_signal` and
  `→ NavigationModule.stop_signal`
- `SLAM.odometry → NavigationModule.odometry` (priority over driver dead-reckoning)
- Autonomy chain: `NavigationModule.waypoint → LocalPlannerModule.waypoint`,
  `TerrainModule.terrain_map → LocalPlannerModule.terrain_map`,
  `LocalPlannerModule.local_path → PathFollowerModule.local_path` and
  `→ SafetyRingModule.path`
- Visual servo: `SemanticPlannerModule.servo_target → VisualServoModule`,
  `VisualServoModule.goal_pose → NavigationModule.goal_pose`,
  `VisualServoModule.nav_stop → NavigationModule.stop_signal`
- `cmd_vel` mux: `Teleop / VisualServo / Navigation.recovery_cmd_vel /
  PathFollower → CmdVelMux → driver.cmd_vel`. `SafetyRingModule.cmd_vel`
  observes the mux output.
- Localization health: `SlamBridgeModule.localization_status →
  SafetyRing / Navigation / DepthVisualOdom`
- Visual odometry fusion: `DepthVisualOdomModule.visual_odometry →
  SlamBridgeModule.visual_odom`
- TraversabilityCost fan-in/out: `OccupancyGrid / ElevationMap / ESDF /
  Terrain → TraversabilityCostModule → Navigation.costmap +
...
GoalResolver._try_resolve(scene_graph_json, instruction)
   ├─ Tag lookup     (TaggedLocationStore exact / fuzzy)
   ├─ Fast Path      (keyword + CLIP fusion, ~0.17 ms, threshold 0.75)
   ├─ Vector memory  (CLIP embedding search in ChromaDB or numpy fallback)
   ├─ Frontier       (FrontierScorer information-gain on TopologySemGraph)
   └─ Visual servo   (VLM bbox detection + PD tracking, near/far split)
   ↓
goal_pose → NavigationModule
   ↓
GlobalPlannerService.plan() → WaypointTracker → waypoint stream
   ↓
LocalPlannerModule → PathFollowerModule → CmdVelMux → driver
```
...
- `~/data/nova/maps/<name>/` (legacy) or `~/data/lingtu/maps/<name>/` (new
  default) — `map.pcd`, `tomogram.pickle`, `map.pgm` + `map.yaml`,
  `patches/`, optional `map.pcd.predufo` (DUFOMap pre-clean backup).
  Override with `NAV_MAP_DIR`.
- `.lingtu/run.{pid,json}` — daemon lifecycle (`cli/run_state.py`).
- `logs/<timestamp>_<profile>/lingtu.log` — per-run log
  (`cli/logging_util.py`).

## Sensor calibration

The `calibration/` toolbox covers camera intrinsics (OpenCV checkerboard),
IMU Allan variance, LiDAR–IMU extrinsics (HKU MARS LI-Init), and
...
- [`AGENTS.md`](../AGENTS.md) — primary engineering reference
- [`MODULE_FIRST_GUIDELINE.md`](../MODULE_FIRST_GUIDELINE.md) — architectural rules
- [`PLANNER_SELECTION.md`](./PLANNER_SELECTION.md) — PCT vs A* decision
- [`gnss_integration.md`](./gnss_integration.md) — GNSS / RTK fusion
- [`TASK_ORCHESTRATION.md`](./TASK_ORCHESTRATION.md) — multi-agent task triggering
- [`TOPIC_CONTRACT.md`](./TOPIC_CONTRACT.md) — ROS2 topic contract used by C++ nodes
