...
def _build_scenario(scenario_name: Optional[str], scenario_args: dict):
    ...
    from simulate.scenarios.navigation import NavigationScenario
    from simulate.scenarios.semantic_nav import SemanticNavScenario
...
def _build_mujoco_engine(world: str, headless: bool):
    ...
    from simulate.worlds.registry import get_world
    try:
...
class _MuJoCoEngine:
    """最小化 MuJoCo 引擎，实现 SimEngine 协议供 SimROS2Bridge 调用."""
...
def run_simulation(
    ...
    from simulate.worlds.registry import list_worlds
    worlds = list_worlds()
    ...
    bridge_thread = None
    try:
        from simulate.bridge.ros2_bridge import SimROS2Bridge
