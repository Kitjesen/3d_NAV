...
class RobotConfig:
    ...
    单一真源：物理参数、初始姿态、策略路径、关节映射全部在此定义。
    与 config/robot_config.yaml 的 sim 节对应。
    """

    # 模型文件
    robot_xml: str = ""           # robot.xml 路径（相对于 sim/ 目录或绝对路径）
    policy_onnx: str = ""         # policy.onnx 路径
    ...
    # 物理参数（来自 CLAUDE.md 全局记忆）
    pd_kp: float = 65.0           # 位置增益（hip）
    pd_kv: float = 5.0            # 速度增益（阻尼）

    # 策略参数（与 brainstem StandardObservationBuilder 一致）
    ...
    policy_freq_hz: float = 50.0  # 策略推理频率
    obs_dim: int = 57
    history_len: int = 5

    # 站立姿态 (Dart 顺序，直接用，不取反)
    ...
    # 关节顺序转换表（来自 nova_nav_bridge.py 原始常量）
